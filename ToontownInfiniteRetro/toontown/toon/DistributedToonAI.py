    ...
    def verifyDNA(self):
        ...
        if self.isPlayerControlled():
            ...
            if self.dna.legColor not in allowedColors:
                self.dna.legColor = allowedColors[26]
                changed = True
            if self.dna.armColor not in allowedColors:
                self.dna.armColor = allowedColors[26]
                changed = True
            if self.dna.headColor not in allowedColors:
                self.dna.headColor = allowedColors[26]
                changed = True
...
def fires(count):
    ...
    invoker = spellbook.getInvoker()
    if not 0 <= count <= 1000:
        return 'Your fire count must be in xrange (0-1000).'
...
def spawn(command, cogType=None, cogLevel=None, isV2=0, isWaiter=0):
    """
    Spawns a single cog, an invasion, a building or a field office.

    Cog names are based on the suitHeadTypes list in SuitDNA.py

    Example usage:

    ~spawn cog [cog name] [cog level]
    ~spawn building [cog name]
    ~spawn invasion [cog name] [isSkelecog] [isV2] [isWaiter]
    ~spawn end (ends an invasion)
    ~spawn cogdo [level]
    """
    invoker = spellbook.getInvoker()
    ...
    if command != 'cogdo' and cogType:
        try:
            cogIndex = SuitDNA.suitHeadTypes.index(cogType)
        except:
        ...
        if invMgr.getInvading():
            ...
            return 'No active cog invasion!'

    if command == 'invasion':
        dept = SuitDNA.getSuitDept(cogType)
        deptIndex = SuitDNA.suitDepts.index(dept)
        relIndex = SuitDNA.getSuitsInDept(deptIndex).index(cogType)
        invoker.doCogInvasion(deptIndex, relIndex, cogLevel, isV2, isWaiter)
        return
    elif command == 'cog':
        if cogLevel:
            try:
                baseLvl = SuitBattleGlobals.SuitAttributes[cogType]['level']
                testLvl = cogLevel - baseLvl - 1
                SuitBattleGlobals.SuitAttributes[cogType]['hp'][testLvl]
            except:
                return 'Invalid cog level!'
        returnCode = invoker.doSummonSingleCog(cogIndex, cogLevel)
    elif command == 'building':
        returnCode = invoker.doBuildingTakeover(cogIndex)
    elif command == 'cogdo':
        cogIndex = int(cogType)
        returnCode = invoker.doCogdoTakeOver(cogIndex, 1)
    else:
        return 'Invalid command.'

    return 'Result: {0}'.format(returnCode[0])
